...
SELECT following_user_id FROM follower INNER JOIN user ON user.user_id=follower.follower_user_id WHERE user.user_name="${username}";`;
const followingPeopleIds=await db.all(getTheFollowingPeopleQuery);
const arrayOfIds=followingPeople.map(eachUser)=>eachUser.following_user_id);
return arrayOfIds;
};
//AUTHENTICATION USER
const authenticationUser=(request,response,next)=>{
let jwtToken;
...
const getTweetQuery=`SELECT * FROM tweet INNER JOIN follower ON tweet.user_id= follower.following_user_id WHERE tweet.tweet_id="${tweetId}" AND follower_user_id='${user_id}';`;
...
if (password.length <6){ response.status(400); response.send("Password is too short"); }else{ const hashedPassword=await bcrypt.hash(password,10); const createUserQuery=`INSERT INTO user(username,password,name,gender) VALUES( "${username}" ,"${gender}","${hashedPassword}","${name}")`; await db.run(createUserQuery); response.send("User Created successfully"); } } }); //API 2 app.post("/login/",async(request,response)=>{
    ...
    //API 7
    app.get("/tweets/:tweetId/likes/",authenticationUser,tweetAcessVerification,async(request,response)=>{
    const {tweetId}=request.params;
    ...
    //API -8
    app.get("/tweets/:tweetId/replies/",authenticationUser,tweetAcessVerification,async(request,response)=>{
    const {tweetId}=request.params;
    ...
    //API 9
    app.get("/user/tweets/",authenticationUser,async(request,response)=>{
    const {userId}=request;
    ...
    //API 10
    app.post("/user/tweets/",authenticationUser,async (request,response)=>{
    ...
    //API 11
    app.delete("/tweets/:tweetId/",authenticationUser,async(request,response)=>{
    const {tweetId}=request.params;
    const {userId}=request;
